...
function generate_pca!(df::DataFrame)
    # Convert ASVAB columns to matrix
    asvabs = Matrix(df[:, end-5:end])
    # Fit PCA model
    M = fit(PCA, asvabs; maxoutdim=1)
    asvabsPCA = MultivariateStats.transform(M, asvabs)
    # AddPC to DataFrame
    df = @transform(df, :asvabPCA = asvabsPCA[:])
    return df
end 
...
function factor_model(θ::Vector{T}, X::Matrix, Xfac::Matrix, Meas::Matrix, 
    ...
    for r in 1:R
        # ASVAB test j, compute:
        Mlike = zeros(T, N)
        for j in 1:J
            Mres= Meas[:, j] .- Xfac * γ[:, j] .- α[j] * ξ[r]
            Mstd= sqrt(σ[j]^2)
            Mlike[:, j] = (1 ./ Mstd) .* pdf.(Normal(0, 1), Mres ./ Mstd)
           # Mlike[:, j] = pdf.(Normal(Mres, Mstd))
        end
        # Wage
        ...
        like += ω[r] .* prod(Mlike, dims=2) .* Ylike .* φ(ξ[r])
    end
...
function run_estimation(df::DataFrame, start_vals::Vector)
    ...
    td = TwiceDifferentiable(θ -> factor_model(θ, X, Xfac, asvabs, y, 9), start_vals; autodiff = :forward)
    result = optimize(td, start_vals, Newton(linesearch = LineSearches.BackTracking()), 
                Optim.Options(g_tol=1e-6, f_tol=1e-6, iterations=1000, show_trace=true, show_every=10))
...
function main()
    ...
    println("="^80)
    OLSonASVAB = lm(@formula(logwage ~ black + hispanic + female + schoolt + gradHS + grad4yr), df)
    println(OLSonASVAB)
    ...
    println("multicolinearity due to high correlation among ASVAB scores")
