...
function variance_quadrature()
    ...
    println("more grid points yields better approximation")
...
function mixed_logit_quad(theta, X, Z, y, R)
    ...
    #This involves integreting over the distribution of gamma

    # Initialize integrated probabilities
    T = promote_type(eltype(X), eltype(theta))
    P_integrated = zeros(T, N, J)
    
    #Loop through grid points to do summation via loops
    for r in eachindex(nodes)
        num_r = zeros(T, N, J)
         # Compute choice probabilities for this gamma_r
        for j = 1:J
...
function mixed_logit_mc(theta, X, Z, y, D)
    ...
    # Weight and add to integrated probabilities
    density_weight = weights[(b - a) / D] * pdf(Normal(mu_gamma, sigma_gamma), draws[d])
    P_integrated .+= P_d * density_weight
    end
    # compute loglike calculation
    loglike = -sum(bigY .* log.(P_integrated))
...
function optimize_mlogit(X, Z, y)
    ...
    #initialize twice differentiable object
...
#using autodiff
    ...
    #evaluate the hessian at the estimate
...
function optimize_mixed_logit_quad(X, Z, y)
    ...
    result = optimize(theta -> mixed_logit_quad(theta, X, Z, y, 7),
                     startvals, LBFGS(),
                     Optim.Options(g_tol = 1e-5, iterations=100_000, show_trace=true);
                     autodiff = :forward)
...
function allwrap()
    ...
    println("same interpretation as PS3")
    println("Compare γ̂ to PS3 results and interpret")
...
# Run the main function to execute all steps
 allwrap()
